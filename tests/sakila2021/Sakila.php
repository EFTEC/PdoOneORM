...
class Sakila extends _BasePdoOneRepo
{
    ...
    protected static function getPdoOne()
    {
        ...
        if (self::BINARYVERSION !== static::COMPILEDVERSION) {
            $p = new PdoOneORM('test', 'no database', '', '');
            $p->throwError('Repository classes requires a new version. Please update PdoOne and rebuild'
                , self::class);
        }

        if(PdoOneORM::instance(false)!==null) {
            self::$pdoOne=PdoOneORM::instance(false);
            return self::$pdoOne;
        }

        if (function_exists('pdoOneORM')) {
            /** @noinspection PhpUndefinedFunctionInspection */
            self::$pdoOne = pdoOneORM();
            return self::$pdoOne;
        }
        if (isset($GLOBALS['pdoOneORM']) && $GLOBALS['pdoOneORM'] instanceof PdoOneORM) {
            self::$pdoOne = $GLOBALS['pdoOneORM'];
            return self::$pdoOne;
        }
        return self::$pdoOne;
    }
