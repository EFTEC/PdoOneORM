...
class PdoOne_mysql_sakila2021_Test extends TestCase
{
    ...
    {
        $this->pdoOne = new PdoOneORM('mysql', '127.0.0.1', 'root', 'abc.123', 'sakila_lite');
        $this->pdoOne->connect();
    ...
    public function testUpdate2(): void
    {
        ...
        $this->assertEquals(['city_id' => 1,
           ...
           'country_id' => 87,
            'last_update' => '2006-02-15 04:45:25.000'], $original);
        $copy = $original;
        unset($copy['city'],$copy['country_id']);
        $copy['city'] = 'Modified';
        $this->assertEquals(['city_id' => 1,
            'city' => 'Modified',
            'last_update' => '2006-02-15 04:45:25.000'], $copy);
        // partial update
        $this->assertNotFalse(CityRepo::update($copy));
        $read = CityRepo::first(1);
        $this->assertEquals(['city_id' => 1,
